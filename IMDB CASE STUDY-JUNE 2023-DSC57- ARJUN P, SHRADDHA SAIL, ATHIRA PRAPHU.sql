...
-- Type your code below:
SELECT g.genre, COUNT(g.genre) AS number_of_movies, MAX(m.year) AS max_year
FROM movie AS m
...
+---------------+-------------------+ */
-- Type your code below:
SELECT g.genre,ROUND(AVG(m.duration),2) AS duration_avg
FROM movie AS m
INNER JOIN genre AS g
ON m.id=g.movie_id
GROUP BY g.genre;
...
RANK () OVER ( ORDER BY COUNT(genre) DESC) AS rank_of_genre
FROM genre
GROUP BY genre)
...
+---------------+-------------------+---------------------+*/
-- Type your code below:
SELECT m.title,g.genre, AVG(r.avg_rating) AS avg_rating
FROM movie AS m
INNER JOIN ratings AS r
ON m.id=r.movie_id
INNER JOIN genre AS g
...
-- Type your code below
SELECT COUNT(r.movie_id) AS movies
FROM ratings AS r
INNER JOIN movie AS m 
ON m.id=r.movie_id
...
FROM director_mapping AS d
       ...
       ON d.name_id=n.id
       INNER JOIN genre_top3 
       ON g.genre=genre_top3.genre
       INNER JOIN ratings AS r
